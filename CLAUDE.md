...
- `types` — domain model classes (`Board`, `Cell`, `Tile`, `Result`, `Config`, `Locale`, …) plus `*Json` shapes. Most of these have a `fromJson` / `toJson` round-trip — use them at the wire boundary instead of hand-rolled serialization.
- `configs` — split into **games** (`scrabble`, `superScrabble`, `scrabbleDuel`, `letterLeague`, `crossplay`, `literaki`, `kelimelik`, `duplicatEliptic`, `duplicatCompletiv` — each defines board size, bonuses, rack size, blanks count, bingo bonus) and **languages** (`english`, `french`, …, each spreads a base game config and overrides `locale` + `tiles`). A locale config is one game × one language; e.g. `polishScrabble` = `scrabble` ⊕ Polish tiles + digraphs. Adding a language means a new config here **and** the 15-step checklist under "Add a new language" in `README.md`.
  - `duplicatEliptic` is the Romanian federation variant: `rackSize: 8` with `maximumWordLength: 7`. `Config.maximumWordLength` defaults to `rackSize`, so it changes nothing for the other games. It bounds the **number of tiles placed from the rack**, not the word's length — words still run through board tiles and can be longer. `MoveGenerator` uses it for both the placement cap (`maxPlacements = min(rackTotal, maximumWordLength)`) and the bingo trigger. `ReferenceSolver`, the brute-force oracle the solver tests compare against, mirrors both rules — change one and you must change the other or the parity tests diverge.
...
- **State**: Redux Toolkit + Redux-Saga. Slices in `src/state/{board,cellFilters,dictionary,draws,hoveredTile,rack,results,settings,solve,verify}`, each exporting `<name>Slice` (reducer + actions) and selectors. The root saga in `state/sagas.ts` reacts to slice actions: `submit` → call SDK → write results back. `solve`, `verify`, and `dictionary` use `takeLatest` (only the latest in-flight request resolves); cell/rack edits use `takeEvery`. State is intentionally **not** serializable-checked (`serializableCheck: false`) because slices hold class instances (`Board`, `Tile`).
- **SDK layer (`src/sdk/`)**: thin browser/server clients for four of the five API routes — `solve`, `verify`, `visit`, and `dictionary/[locale]/[word]`. The binary `dictionary/[locale]` download is the exception: the service worker's dictionary loader fetches it directly. `findWordDefinitions` is memoized at the saga level via `lib/memoize`. Otherwise always go through SDK — never `fetch` directly from a saga or component.
- **Duplicat Completiv game** (`Game.DuplicatCompletiv`): its own game, not a mode. `configs/src/games/duplicatCompletiv.ts` is `scrabble` with `rackSize: 6` and `maximumWordLength: 7` — the rack holds only the six fixed tiles, and `maximumWordLength` lifts both the placement cap and the bingo threshold to cover the drawn seventh. Selecting it swaps `Results` for `components/DrawResults` and routes `onSolve` to a sweep: one solve per tile still in the bag. Pieces: `getDrawCandidates`/`getRemainingTiles` (`state/lib.ts`) compute the bag and the blank rules; `pickBestDrawResults` (`lib/`) is the shared "this move needed the draw" filter used by both the worker and the server fallback — it returns every top-scoring move, so exact ties all survive; `solver-worker/solveDraws.ts` runs the sweep; results land in the `draws` slice. Do **not** loop `solveLocally` — the worker tracks a single `latestSolveId` and each solve supersedes the previous, which is why the sweep is one `solve-draws` message. `selectNavigableResults` is what the compact mobile controls step through, so they work in both modes. `onInitialize` reconciles rack length unconditionally, or a rack persisted under another game would leave this one holding more than six tiles.
...
- Only `dictionaries`, `solver`, `types`, `word-definitions`, and `scrabble-solver` have a `test` script. Tests are auto-discovered under `src/` matching `*.test.ts(x)`. The 180s timeout is needed because some solver tests build a real `Gaddag` from a downloaded dictionary.
...
- `e2e.yml` — Cypress (`cypress-io/github-action`) against the production server on :3333. Uploads screenshots on failure.
